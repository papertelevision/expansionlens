 ...
 */
async function fetchGoogleCompetitors(lat, lon, type = 'dentist', radius = 5633) {
  const key = API_KEY();
  if (!key) return [];
  ...
  for (let page = 0; page < 3; page++) {
    try {
      let url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lon}&radius=${radius}&type=${type}&key=${key}`;
      if (nextPageToken) {
 ...
 */
export async function enrichCompetitors(competitors, lat, lon, type = 'dentist', fallbackName = 'Unnamed Dental Practice', radiusMeters = 5633) {
  const key = API_KEY();
  if (!key) return { competitors, avgRating: null, totalReviews: null };

  const googleResults = await fetchGoogleCompetitors(lat, lon, type, radiusMeters);
