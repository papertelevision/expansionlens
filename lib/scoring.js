...
export function calculateScore({ population, medianIncome, competitorCount, walkScore, collegePercent, employmentRate, popGrowth, avgCompetitorRating, config, searchRadius }) {
  ...
  const internal = [
    ...
    { label: 'Competition', earned: getCompetitionScore(population, competitorCount, cfg, searchRadius), max: cfg.competition.max },
...
// Census tract population covers ~1-2 sq mi; search area varies by density.
// We estimate area population by scaling tract density across the search area.
function getCompetitionScore(population, competitorCount, cfg, searchRadius) {
  if (competitorCount === 0) return cfg.competition.max;
  const areaMultiplier = searchRadius?.areaMultiplier || cfg.areaMultiplier || (25 / 1.5);
